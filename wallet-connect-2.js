...
const vueApp = createApp({
	...
	methods: {
		...
		pair: async function(wcUri) {
			...
			await walletConnectService.core.pairing.pair({ uri: wcUri });
		...
		onRPCRequest: async function(session, request, response) {
			...
			const whitelistedMethods = ['wc_authRequest', 'bch_getTokens_V0', 'bch_getBalance_V0', 'bch_getChangeOutput_V0'];
...
}).mount('#cashrpc-over-wc-container');
